...
SET regulation_id = (
    SELECT id FROM printnread_regulation WHERE code = 'R22' LIMIT 1
)
...
-- SELECT COUNT(*) as total_subjects FROM printnread_subject;
-- SELECT COUNT(*) as subjects_with_regulation FROM printnread_subject WHERE regulation_id IS NOT NULL;
-- SELECT COUNT(*) as subjects_with_semester FROM printnread_subject WHERE semester_id IS NOT NULL;
-- SELECT COUNT(*) as subjects_with_sub_branch FROM printnread_subject WHERE sub_branch_id IS NOT NULL;

-- Show distribution by regulation
-- SELECT r.code, COUNT(s.id) as subject_count
-- FROM printnread_regulation r
-- LEFT JOIN printnread_subject s ON s.regulation_id = r.id
-- GROUP BY r.code
-- ORDER BY r.code;
...
-- FROM printnread_semester sem
-- LEFT JOIN printnread_subject s ON s.semester_id = sem.id
